...
async function recommendhybrid(req, res) {
    try {
        ...
        let message = "Recommendations hybrids generated successfully"; // Cambiado de const a let
        const base_payload = {
        ...
        if (Array.isArray(genre_filter) && genre_filter.length > 0) {
            ...
            message = `Recommendations hybrid filtered by genres: ${genre_filter.join(', ')} generated successfully`;
        }
        ...
        if(response.data.ok !== true){
            throw new Error(response.data.message || "Error in recommendation model hybrid");
        }

        const recommender_ids = response.data.recommendations;

        console.log("Recommendation model hybrid response: ", response.data);
