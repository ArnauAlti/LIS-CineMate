...
async function disableMedia(req, res) {
    try {
        ...
        console.log("(Disable) media_id " + mediaId);
        const query = await userDB.query(
            "UPDATE media SET active = 'false' WHERE id = $1",
            [mediaId]
        );
        if (query.rowCount != 1) {
            if (query.rowCount == 0) {
            ...
            res.status(200).json({message: "Element Disabled"});
        }
    } catch (error) {
        console.log(error);
        res.status(500).json({error: error, message: "Failed to Disable"});
    }
}

module.exports = disableMedia;