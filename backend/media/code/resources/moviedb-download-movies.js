...
async function processMediaInfo(content, isMovie) {
   ...
   if (isMovie) {
      ...
      for (const season of details.seasons) {
         ...
         insert['duration'] = 0;
         season.episodes.map(s => insert['duration'] += s.runtime);
         insert['director'] = director;
         insert['cast'] = cast;
         insert['release'] = String(season.air_date).replaceAll("-", "/");
...
async function setMovies() {
   ...
   try {
      ...
      for (const element of uniqueMediaInfo) {
         ...
         if (result.rowCount == 0) {
            console.log("Info Movie: " + element['movie_db'], " / " + " -> Failed");
            failedInfo++;
         } else {
            // console.log("Info Movie: " + element['movie_db'], " / " + " -> Success");
            addedInfo++;
         }
      }
